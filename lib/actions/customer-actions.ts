...
export const createCustomer = async (formData: FormData) => {
  ...
  const res = await serverFetcher.post(
    `/api/customer/save-all`,
    {
      ...parsedData,
      CustomerOid: uuidv4(),
...
export const updateCustomer = async (formData: FormData) => {
  const oid = formData.get("CustomerOid") as string;

  const res = await serverFetcher.post(
    `/api/customer/update-from-employees?CustomerOid=${oid}`
  );

  return res;
};

export const saveCustomerTaxAccess = async (formData: FormData) => {
  const token = await getAccessToken();
  const raw = Object.fromEntries(formData.entries());
...
export const saveCustomerInsuranceAccess = async (formData: FormData) => {
  const token = await getAccessToken();
  const raw = Object.fromEntries(formData.entries());
